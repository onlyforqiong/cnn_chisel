...
module custom_BusMatrixOutputM1 (
...
// -----------------------------------------------------------------------------
    ...
    input         held_tran_op0;  // Port 0 held_tran_op signal

    input         HREADYOUTM; // HREADY feedback

    output        active_op0;    // Port 0 active_op signal
...
// -----------------------------------------------------------------------------
    ...
    wire        held_tran_op0;  // Port 0 held_tran_op signal
    wire        active_op0;    // Port 0 active_op signal
...
// -----------------------------------------------------------------------------
  ...
  custom_BusMatrixArbiterM1 u_output_arb (
  ...
  assign active_op0 = (addr_in_port == 2'b00) & !no_port;

  // addr_opess/control output decode
  assign i_hselm     = (addr_in_port == 2'b00) & sel_op0 & !no_port;
  assign HADDRM      = ( (addr_in_port == 2'b00) & !no_port ) ? addr_op0 : {32{1'b0}};
  assign i_htransm   = ( (addr_in_port == 2'b00) & !no_port ) ? trans_op0 : 2'b00;
  assign HWRITEM     = (addr_in_port == 2'b00) & write_op0 & !no_port;
  assign HSIZEM      = ( (addr_in_port == 2'b00) & !no_port ) ? size_op0 : 3'b000;
  assign i_hburstm    = ( (addr_in_port == 2'b00) & !no_port ) ? burst_op0 : 3'b000;
  assign HPROTM      = ( (addr_in_port == 2'b00) & !no_port ) ? prot_op0  : {4{1'b0}};
  assign HMASTERM    = ( (addr_in_port == 2'b00) & !no_port ) ? master_op0 : 4'b0000;
  assign i_hmastlockm = (addr_in_port == 2'b00) & mastlock_op0 & !no_port;
  ...
  assign HWDATAM = ( data_in_port == 2'b00 ) ? wdata_op0 : {32{1'b0}};
